...
실시간으로 AI 에이전트들이 작업하는 모습을 픽셀 아트 가상 사무실에서 시각화합니다. Claude Code 에이전트들이 생성되어 책상에 앉고, 커피 브레이크를 가지며, Slack 스타일 채팅 패널에서 대화하는 모습을 볼 수 있습니다. 모두 등각 픽셀 아트 사무실로 렌더링됩니다. 헤르메스 자동화 시스템의 8개 부서 (Tistory 발행, 모닝 브리핑, 자동화 정비, 리서치/트렌드, Notion 동기화, 투자/트레이딩, 미디어/영상, DevOps)와 실시간 cron 상태를 반영하기 위해 **Hermes Control Room**으로 포크되었습니다.
